...
COOKIE_NAME_PREFIX = "Z-"  # identifies a cookie as being one used by gae-sessions (so you can set cookies too)
COOKIE_PATH = "/"
...
class SessionMiddleware(object):
    ...
    """
    def __init__(self, app, cookie_key, lifetime=DEFAULT_LIFETIME, no_datastore=False, cookie_only_threshold=DEFAULT_COOKIE_ONLY_THRESH, expire_msgs_each_call=False):
        self.app = app
        self.lifetime = lifetime
        self.no_datastore = no_datastore
        self.cookie_only_thresh = cookie_only_threshold
        self.cookie_key = cookie_key
        self.expire_msgs_each_call = expire_msgs_each_call
    ...
    def expire_msgs(self):
        session = get_current_session()
        msgs = session.get('msgs', [])
        if msgs != None:
            new = []
            for m in msgs:
                cnt = int(m.get('cnt', 1)) - 1
                if cnt > 0:
                    new.append({ 'msg': m['msg'], 'cnt': cnt, 'cls': m['cls'] })
#                else:
#                    logging.info('EXPIRING msg %s' % m['msg'])
            msgs = new
        session['msgs'] = msgs
